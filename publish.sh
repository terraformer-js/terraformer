...
git push https://github.com/terraformer-js/terraformer.git master

# push the new tag, not the old tags
git push https://github.com/terraformer-js/terraformer.git v$VERSION

# publish each package on npm
lerna publish --skip-git --yes --repo-version $VERSION --force-publish=*

# create a ZIP archive of the dist files
TEMP_FOLDER=terraformer-v$VERSION;
mkdir $TEMP_FOLDER
cp packages/*/dist/* $TEMP_FOLDER
zip -r $TEMP_FOLDER.zip $TEMP_FOLDER
rm -rf $TEMP_FOLDER

# Run gh-release to create a new release with our changelog changes and ZIP archive
gh-release --t v$VERSION --repo terraformer --owner terraformer-js -a $TEMP_FOLDER.zip
